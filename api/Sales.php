...
class Sales{
	...
	public function makeTransaction($data){
		...
		$updatedData = $this->getAllBranchInventoryProduct($branch_id);
	...
	public function getBranchesInvoices($post){
		...
		if(isset($post['sdate']) & isset($post['edate'])){
			if(($post['sdate'] != '') & ($post['edate'] != '')){
				$sql .= " WHERE purchase_date BETWEEN '" . $post['sdate'] . "' AND  '" . $post['branch_id'] . "'";
			}
		}
		// ECHECK IF WHERE EXITS IN THE SQL STATEMENT BEFOR ADDING IT
		// I'LL DO IT LATER
		if(isset($post['branch_id'])){
			if((int)$post['branch_id'] != ''){
				$sql .= " WHERE branch_id = " . (int) $post['branch_id'];
			}
		}
		$sql .= " ORDER BY purchase_date DESC";
